...
def create_sidebar(app):
    ...
    for i, color in enumerate(app.colors):
        ...
        button = tk.Button(
            sidebar, width=4, height=2, command=lambda c=color: app.change_color(c)
        )
    ...
    pick_color_button = ttk.Button(sidebar, text="Pick Color", command=app.pick_color)
    pick_color_button.pack(pady=5, padx=5, fill="x")
    app.custom_color_picker = CustomColorPicker(sidebar, app.change_color)
